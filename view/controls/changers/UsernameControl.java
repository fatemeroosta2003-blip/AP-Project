...
public class UsernameControl {
    ...
    private static void getAPic() throws IOException {
        ...
        if (pane != null) {
            ...
            pane.getChildren().get(pane.getChildren().size() - 1).setLayoutY(340);
    ...
    public void initialize(){
        username.textProperty().addListener((observableValue, s, t1) -> {
            for (User user : User.getUsers()){
                if (user.getUsername().equals(username.getText())) error.setText("This username already exist");
                else error.setText("");
            }
        });
    }
    public void confirmUsername(MouseEvent mouseEvent) throws IOException {
        ...
        if(userCaptchaAnswer == null || userCaptchaAnswer.getText().length() == 0 ||
            ...
            alert.setHeaderText("your username changed successfully");
            alert.showAndWait();
